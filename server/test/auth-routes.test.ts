...
describe('auth routes', () => {
  it('register returns 201 verification_sent with no auth cookie', async () => {
    const res = await app.inject({
      ...
      payload: { name: 'Jithmi', email: 'j@x.com', password: 'secret123' },
    })
  ...
  it('verify logs in', async () => {
    await app.inject({ method: 'POST', url: '/api/auth/register', payload: { name: 'J', email: 'j@x.com', password: 'secret123' } })
