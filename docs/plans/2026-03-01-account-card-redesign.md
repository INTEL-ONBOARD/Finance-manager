# Account Card Redesign Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the flat account info cards in AccountsPage with a physical credit-card style visual (gradient background, chip SVG, type icon, masked number, cardholder name) while keeping the recent transactions and progress bars in an info section below each card.

**Architecture:** Single-file change — `AccountsPage.tsx`. Add a `adjustColor` colour helper inline, a `ChipSVG` component inline, and rewrite the card rendering block. All account data already available; user name comes from `useAuth()`. No new files, no data model changes.

**Tech Stack:** React 19, TypeScript, Framer Motion, Lucide React, inline styles + Tailwind utility classes, CSS variables (var(--*))

---

## Task 1: Rewrite AccountsPage.tsx with physical card design

**Files:**
- Modify: `src/renderer/src/pages/AccountsPage.tsx` (full rewrite of rendering block)

### Step 1: Replace the file with the new implementation

The file lives at:
`/Users/kkwenuja/development/Personal Projects/Finance-manager/.worktrees/accounts-management/src/renderer/src/pages/AccountsPage.tsx`
...
export default function AccountsPage() {
  ...
  // Cardholder display name — last name initial if long, else full name (max 20 chars)
      ...
      {accounts.length > 0 && (
        <div className="grid grid-cols-2 gap-5">
          {accounts.map((acct, i) => {
            ...
            // Gradient colours derived from account colour
            const gradFrom  = adjustColor(acct.color, -60);
            const gradMid   = acct.color;
            const gradTo    = adjustColor(acct.color, 30);
            const cardGradient = `linear-gradient(135deg, ${gradFrom} 0%, ${gradMid} 55%, ${gradTo} 100%)`;
            ...
            return (
              ...
              >
                {/* ⋯ menu trigger — floats above card */}
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    background: 'rgba(255,255,255,0.08)', top: -30, right: -30, pointerEvents: 'none',
                  }} />
                  <div style={{
                    position: 'absolute', width: 80, height: 80, borderRadius: '50%',
                    background: 'rgba(255,255,255,0.06)', bottom: 10, left: -20, pointerEvents: 'none',
                  }} />
                  ...
                  {/* Row 3: cardholder + valid thru */}
                  <div style={{ display: 'flex', gap: 24, alignItems: 'flex-end' }}>
                    <div>
                      <div style={{ fontSize: 8, color: 'rgba(255,255,255,0.55)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 2 }}>
                    ...
                    <div style={{ flex: 1 }} />
                    {/* Bank/type label top-right of bottom row */}
                    <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', textAlign: 'right', maxWidth: 80, lineHeight: 1.3 }}>
                      {acct.bank ? acct.bank.split(' ').slice(0, 2).join(' ') : TYPE_LABEL[acct.type]}
                    </div>
                  </div>

                  {/* Row 4: masked number + network */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 2 }}>
                    <div style={{
                      fontSize: 13, fontFamily: 'Geist Mono, monospace', color: 'rgba(255,255,255,0.85)',
                ...
                <div style={{ padding: '12px 16px 14px' }}>
                  ...
                  {/* Recent transactions */}
                  {recentTxns.length > 0 ? (
                    <div style={{ borderTop: creditUtil !== null || loanProgress !== null ? '1px solid var(--border)' : 'none', paddingTop: creditUtil !== null || loanProgress !== null ? 10 : 0 }}>
...
```bash
cd "/Users/kkwenuja/development/Personal Projects/Finance-manager/.worktrees/accounts-management"
...
```
✓ built in ~Xms   (main)
✓ built in ~Xms   (preload)
✓ built in ~Xs    (renderer)
```

Zero errors. The same pre-existing TS warnings in AuthContext are acceptable.

### Step 3: Commit

```bash
cd "/Users/kkwenuja/development/Personal Projects/Finance-manager/.worktrees/accounts-management"
git add src/renderer/src/pages/AccountsPage.tsx
git commit -m "feat: redesign account cards with physical card visual, gradient, chip SVG, and cardholder info"
```
...
1. `npm run dev` from worktree — app opens, no console errors
2. Navigate to `/accounts`
3. Each account card shows:
   - Gradient background derived from account color (distinct for each card)
   - Gold chip SVG (top-left of card face)
   - Type icon top-right (Wallet for checking, PiggyBank for savings, etc.)
   - Large balance in monospace white text
   - "Current Balance" / "Outstanding" / "Portfolio Value" label
   - Cardholder name (logged-in user's name)
   - Masked card number `•••• •••• •••• ••••`
   - For credit: VISA / Mastercard / Amex badge
   - Below card: recent transactions + credit utilization or loan repayment bar if applicable
4. `⋯` menu still works for edit and delete
5. Adding a new account shows the new card style immediately
6. Both light and dark themes render correctly (card always has white text over gradient)
...
| File | Change |
|------|--------|
| `src/renderer/src/pages/AccountsPage.tsx` | Full rewrite — new card visual, helpers, sub-components |

## Reused Utilities

- `formatCurrency` from `src/renderer/src/utils/formatCurrency.ts`
- `useFinance()` from `src/renderer/src/context/FinanceContext.tsx`
- `useAuth()` from `src/renderer/src/context/AuthContext.tsx` (for cardholder name)
- `AddAccountModal` from `src/renderer/src/components/modals/AddAccountModal.tsx`
- Framer Motion `motion`, `AnimatePresence`
- Lucide React icons
- CSS variables: `var(--accent-green)`, `var(--accent-red)`, `var(--accent-brand)`, `var(--text-muted)`, `var(--text-secondary)`, `var(--text-primary)`, `var(--border)`, `var(--bg-card)`
