...
export default function UnsubscribePage() {
  ...
  useEffect(() => {
    ...
    window.electron?.db.unsubscribeMonthly(token)?.then(() => {
      setMsg('You have been unsubscribed from monthly summary emails.')
      setStatus('success')
    }).catch(() => {
      setMsg('Could not update your preferences. The link may be invalid.')
      setStatus('error')
    })
  }, [searchParams])
    ...
    <div className="flex min-h-screen w-full items-center justify-center" style={{ background: 'var(--bg-primary)' }}>
      ...
      >
        ...
        <div className="flex items-center gap-3 mt-4">
          {status === 'pending' && <Loader size={20} className="text-lime-500 animate-spin" />}
          {status === 'success' && <CheckCircle size={20} style={{ color: '#4ade80' }} />}
          {status === 'error' && <XCircle size={20} style={{ color: '#f87171' }} />}
          <p style={{
